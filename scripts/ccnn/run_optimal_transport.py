...
def build_model_data(cc):
    ...
    num_cells = torch.Tensor(
        [
            x.shape[0],
            cc._num_cells_at_rank(1),
            cc._num_cells_at_rank(2),
            cc._num_cells_at_rank(3),
        ]
    ).long()
    ...
    x = [
        x,
        ak.to_torch(cc.cells.features[cc.cells.rank == 1]).float().to(cc.device),
        ak.to_torch(cc.cells.features[cc.cells.rank == 2]).float().to(cc.device),
    ]
...
if __name__ == "__main__":
    ...
    weights = torch.load(
        osp.join(CONFIG.model, run_name, f"{run_name}_{extra_info}.pt"),
        weights_only=True,
    )
